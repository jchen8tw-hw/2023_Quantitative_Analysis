...
for(i in 1:5){
  ...
  cv.error[i] = cv.glm(Wage, poly.fit, K=50)$delta[1]
}
min(cv.error)
which.min(cv.error) # answers differ depending of how the data were split, but the result is most likely to be 4.
...
for(i in 1:4){
  ...
  cv.error[i] = cv.glm(Wage, ns.fit, K=10)$delta[1]
}
plot(seq(1:4),cv.error[1:4])
min(cv.error)
which.min(cv.error) 

qSmall = quantile(Wage$age,.05*which.min(cv.error))
qLarge = quantile(Wage$age,1-.05*which.min(cv.error))
...
plot(age,wage,col="gray")
lines(age.grid,pred.ns$fit,lwd=2,col='red') # The fitted line of a natural cubic spline
lines(age.grid,pred.bs$fit,lwd=2,col='blue') # The fitted line of a cubic spline
